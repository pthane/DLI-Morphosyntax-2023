...
--
- This satisfies open materials requirements for publications along with a list of stimuli or interview questions, etc. in your manuscript
...
## (Notice the use of parentheses. The name of the file begins in the same place where the .Rproj file is stored. We will use the adult HSs' production data as an example.)
...
---
# Sharing Your `xaringan` RSlides with GitHub
...
--
- Every GitHub repository has settings. To use the settings, .


--
- When uploaded to a GitHub repository, the resulting index.html file will turn into a public version of your slides!


---
# 2. Thoughtful graphing/plotting
...
---
# Individual Differences: Code

```{r, calculate-subj-ind-diffs, echo = TRUE}
## Bring EPT and FCT data together
Diffs = left_join(EPT_Participant_Averages, FCT_Participant_Averages, by = "Part_ID", "Age") %>%

## Rename the age column, as it got renamed during the "let_join" operation
  rename(Age = Age.x) %>% 

## Select only the desired columns to make it easier to interpret
  select(Part_ID, Age, Structure_Percentage.x, Structure_Percentage.y) %>% 
  
## Rename columns with similar names
  rename(Production = Structure_Percentage.x,
         Preference = Structure_Percentage.y) %>% 


## Remove SDBA data -- this is about HS performance
  filter(!Age == "SDBA")


## Re-order groups so that they appear in increasing order
Diffs$Age <- factor(Diffs$Age, levels = c("HS5", "HS7/8", "HSA"))
```


---
# Individual Differences: Graph
