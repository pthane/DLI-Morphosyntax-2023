---
title: "Exemplifying Quantitative Research in Bilingual Education: Heritage Language Acquisition in Dual-Language Immersion"
author: "Patrick D. Thane<br/>pthane@umass.edu"
institute: "Western Massachusetts Bilingual Hub"
...
DOM_Master$Group <- factor(DOM_Master$Group, levels = c("SDBA", "MLS-5", "DLI-5", "MLS-7/8", "DLI-7/8"))
...
Subj_Master$Group <- factor(Subj_Master$Group, levels = c("SDBA", "MLS-5", "DLI-5", "MLS-7/8", "DLI-7/8"))
...
--
- There are multiple methods of DLI:

--
  + One-way immersion: all students are home speakers of a language other than English

--
  + Two-way immersion: home speakers of English *and* home speakers of another language (usually Spanish) come together to learn in both languages
...
--
- It is unclear if HSs show growth in their knowledge of Spanish into adolescence, or if they lose their heritage language due to less exposure


--
  + This likely depends on each individual

--
  + What, then, are the factors that can account for individual variability?

--
- Age is difficult to interpret:
...
--
- Do patterns of exposure to the language matter?


--
  + Proficiency (overall knowledge)

--
  + Frequency of use of the heritage language (exposure matters; Putnam & Sánchez, 2013)
...
--
  + This structure doesn't really affect meaning in Spanish, but because it is infrequent, it is a good way to test the power of bilingual education on language development
...
--
  + Example: ve Juan (Juan sees) versus ve *a* Juan (she sees Juan)
...
--
  + This structure has important impact on meaning in Spanish


---
# Little By Little…

```{r, include-brain-overload, height = 10, fig.width = 16, fig.align = 'center', fig.retina = 3}
Overload <- "https://media.licdn.com/dms/image/D4D12AQEhxeRkMDhcVQ/article-cover_image-shrink_600_2000/0/1686833097722?e=2147483647&v=beta&t=MoDETgA3tTvqRUwC0uKQqr5YBgQVt9aPZbulVH_ktls"

knitr::include_graphics(Overload)
```
...
--
- Dual-language immersion is a powerful tool for teaching bilingual children


--
- We know quite little about the role of how DLI shapes heritage language development


--
- There are multiple theories of HL acquisition, and studying multiple age groups of children can help us to figure out which theories are best for our data


--
- The subjunctive and DOM seem to be difficult to learn, so would DLI make a particularly noticeable impact on these structures?
...
--
1. Prepare and pilot the experiment with children not in the study (I practiced with my kiddos in Madrid!)


--
2. Administer the experiment online to adults, gather consent from children's parents


--
3. Administer the experiment online while with students in their schools


--
1. Create an Excel document analyzing every student's response (over 10,000 data points collected)


--
  + Report binary variable (1 = subjunctive/DOM used; 0 = anything else; no score if the participant didn't respond)

--
  + Code additional variables: frequency of use of Spanish, score on a grammar test, etcetera


--
2. Use *R* (R Core Team, 2022) to make graphs and conduct statistical analyses
...
--
- The golden question in quantitative research: **What is the effect of X on Y?**


--
- Dependent variable (*y*): use of subjunctive/DOM (see above)
...
--
  + Task (SCT, production versus MST, receptive)

--
  + School type (DLI versus MLS/monolingual school)

--
  + Age group (5th grade versus 7th/8th grade)

--
  + Frequency of use of Spanish (sum of 6 Likert scales; possible score 0-30)
...
DOM_Overall <- glmer(DOM_Use ~ Group + Task + Use_Joint_Std + Task:Use_Joint_Std +
                       (1 | Part_ID) + (1 | Item),
                     data = DOM_Master_HS,
                     family = "binomial")
```
...
DOM_Overall_Plot <- plot_model(DOM_Overall, show.values = TRUE, show.intercept = TRUE, value.offset = .3, transform = NULL, vline.color = "black") +
                scale_y_continuous(breaks = seq(-6, 6, 2),
                     limits = c(-6, 6)) +
            scale_x_discrete(labels = c("MST : Freq. of use", "Frequency of use of Spanish", "Morphology selection task (MST)", "DLI-7/8 group", "MLS-7/8 group", "DLI-5 group", "(Intercept)")) +
...
DOM_Pairwise <- emmeans(DOM_Overall, spec = "Group")
...
--
- Whenever the *p* value is less than .05, you can anticipate that the difference is "real" ("significant")


--
- Therefore, the following differences are significant:
...
DOM_Diffs$Group <- factor(DOM_Diffs$Group, levels = c("SDBA", "MLS-5", "DLI-5", "MLS-7/8", "DLI-7/8"))
```
...
--
- It seems that both age and DLI work together because:

--
  + DLI-7/8 were greater than both monolingually-educated groups
...
--
- Lots of individual variability; nobody *never* used DOM, and some were just like Spanish-dominant speakers
...
Subj_Overall <- glmer(Mood_Use ~ Group + Task + Use_Joint_Std + Task:Use_Joint_Std +
                       (1 | Part_ID) + (1 | Item),
                     data = Subj_Master_HS,
                     family = "binomial")
```
...
Subj_Pairwise <- emmeans(Subj_Overall, spec = "Group")
...
--
- No differences between groups have *p* values below .05


--
- There does not seem to be any effect of DLI on the development of the subjunctive


--
- Looking at the graphs, it seems that older children have more knowledge of subjunctive than younger children, so additional stats would be useful comparing age groups without considering school group


---
# Subjunctive: Age-Only Comparisons

```{r, isolate-subj-age-only}
Subj_Age_Data <- Subj_Master_HS
Subj_Age_Data$Age <- factor(Subj_Age_Data$Age, levels = c("7th/8th", "5th"))

Subj_Age <- glmer(Mood_Use ~ Age +
                    (1 | Part_ID) + (1 | Item),
                  data = Subj_Age_Data,
                  family = "binomial")

Subj_Age_Pairwise <- emmeans(Subj_Age, spec = "Age")
Subj_Age_Tukey <- contrast(Subj_Age_Pairwise, method = "pairwise")
Subj_Age_Table <- summary(Subj_Age_Tukey)
```

```{r, print-subj-age-Tukey-table}
kable(Subj_Age_Table[1:6], col.names = c("Contrast", "Estimate", "SE", "df", "t ratio", "p"), align = "lccccc")
```


--
- The *p* value for the difference between 7th/8th grade and 5th grade heritage speakers is below .05, so there is a "significant" effect for age in the subjunctive data


--
- This could not be observed by comparing the four groups to one another, so it was acceptable to keep running stats in this instance
...
Subj_Diffs$Group <- factor(Subj_Diffs$Group, levels = c("SDBA", "MLS-5", "DLI-5", "MLS-7/8", "DLI-7/8"))
```
...
--
- No role of bilingual education, but children's knowledge of subjunctive increases with age
...
--
- Bilingual education affected the use of DOM, but not frequency of use

--
- Overall frequency of use subtly affected the subjunctive, but not bilingual education
...
--
- Different variables affect different structures
...
--
- This supports theories of HL acquisition (Putnam & Sánchez, 2013)


--
- Students may understand more than they hear, which emphasizes that **input is necessary, but it's not enough**

--
  + This has long been argued for second language acquisition in immersion programs (Swain, 1993, 1995)

--
  + Simply providing instruction in Spanish isn't enough! We need children to be producing in immersion classrooms, which is a real challenge…
...
--
- Age *and* bilingual education appear to interact in the acquisition of DOM

--
  + Neither variable alone appears to matter by itself, but more statistics would be needed to back up this claim

--
  + Such an effect is called an *interaction* because there is a relationship between two independent variables (that is, X sometimes affects Y)
  
  
--
- Age matters in children's mastery of the subjunctive


--
- The hypothesis is mostly supported, except for the fact that age did not matter "across the board" in the acquisition of DOM
...
--
- These results tell us one of two things:

--
  + The theory makes incorrect predictions regarding frequency of use and exposure (unlikely, given extensive previous evidence)

--
  + DLI classrooms are not promoting the use of Spanish in the way that we anticipate
...
--
  + There is often not a systematic focus on language objectives across the content areas in immersion programs
...
---
# Concluding Remarks: Critiquing This Project
...
--
  + Will translanguaging truly accomplish this goal?

--
  + How can we leverage all students' funds of knowledge while also trying to reinforce the status of the Spanish language, leading to its maintenance?
...
Swain, M. (1995). Three functions of output in second language learning. In G. Cook & B. Seidlhofer (Eds.), *Principle & practice in applied linguistics: Studies in honour of H.G. Widdowson* (pp. 125–144). Oxford University Press.
