...
HSA <- read_csv(here("./CSV Files/Adult HS/Adult HS Subjunctive FCT.csv")) %>%
  filter(Item == "FCT-02") %>% 
  mutate(Group = "HSA")

DLI78 <- read_csv(here("./CSV Files/DLI-78/DLI-78 Subjunctive FCT.csv")) %>%
  filter(Item == "FCT-02") %>% 
  mutate(Group = "HS7/8")
...
DLI5 <- read_csv(here("./CSV Files/DLI-5/DLI-5 Subjunctive FCT.csv")) %>%
  filter(Item == "FCT-02") %>% 
  mutate(Group = "HS5")
