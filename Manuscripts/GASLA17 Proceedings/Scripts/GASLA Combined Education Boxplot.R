...
Master$Child_Group <- factor(Master$Child_Group, levels = c("MLS-5", "DLI-5", "MLS-7/8", "DLI-7/8"))


# Create graph
Education_Boxplot <- Master %>% 
  ggplot(aes(x = Child_Group, y = Ratio, fill = Task)) +
  geom_boxplot() +
  facet_grid(rows = vars(Structure)) +
  scale_y_continuous(breaks = seq (0, 100, 20),
                     limits = c(0, 100)) +
  scale_fill_manual(values = c("black", "white")) +
  labs(x = "Group", y = "Percentage of subjunctive/DOM responses", title = "Subjunctive/DOM by Age, Education, and Task") +
...
ggsave(filename = here("./Manuscripts/GASLA17 Proceedings/Graphs/GASLA17 Education Boxplot.pdf"),
       plot = Education_Boxplot,
       device = "pdf",
       width = 6.5,
       height = 3.5,
       units = "in")
