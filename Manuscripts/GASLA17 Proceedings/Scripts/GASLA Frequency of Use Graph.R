...
Use_Plot <- Aggregate %>% 
  ggplot(aes(x = Use_Joint, y = Ratio)) + 
  geom_jitter(mapping = aes(shape = Group)) +
  guides(color = "none") +
  facet_grid(cols = vars(Structure_SPA),
             rows = vars(Task_SPA)) +
  geom_smooth(method = glm) +
  scale_x_continuous(breaks = seq (0, 30, 5),
                     limits = c(-1, 31)) +
  ...
  labs(x = "Frequency of use of Spanish", y = "Percentage of expected morphology", 
       title = "Subjunctive/DOM by Frequency of Use, Group, and Task") +
  theme(axis.title = element_text(face = "bold"),
        plot.title = element_text(hjust = 0.5, face = "bold"),
        legend.title = element_text(face = "bold"),
...
ggsave(filename = "./Manuscripts/GASLA17 Proceedings/Graphs/GASLA17 Frequency of Use Graph.pdf",
       plot = Use_Plot,
       device = "pdf",
       width = 6.5,
       height = 5,
